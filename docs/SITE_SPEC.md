...
- shard は 1ファイル gzip前 1〜2MB 以下を目安とし、超える場合は先頭4桁に深掘りしてよい
...
```json
{
  ...
  "counts": {"S": {"codes": 12345, "events": 45678}, "T": {...}, "C": {...}},
...
```json
{"S": {"h26": {"new": 123, "changed": 456, "abolished": 78}, ...}, "T": {...}, "C": {...}}
```
...
1. **トップ**: 検索ボックス + マスター種別タブ(S/T/C) + サマリ表(世代別 新設/変更/廃止件数)
   + データの注意書き(§5)
